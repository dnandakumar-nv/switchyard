...
"""Tests for handler-preserving Switchyard routing middleware."""
...
async def test_async_middleware_routes_through_replacement_model_and_handler() -> None:
    ...
    middleware = SwitchyardRoutingMiddleware(algorithm)  # type: ignore[arg-type]
...
async def test_sync_model_call_inside_event_loop_directs_user_to_ainvoke() -> None:
    middleware = SwitchyardRoutingMiddleware(RecordingAlgorithm())  # type: ignore[arg-type]
