# Switchyard routing middleware for LangChain Deep Agents
...
- Python 3.12 or newer. The checked-in lockfile targets Python 3.12.
- A checkout of this Switchyard repository.
...
```bash
uv sync \
  --project examples/experimental/langchain-deepagents \
  --locked \
  --python 3.12 \
  --extra openrouter
```
...
- the local checkout of `nemo-switchyard` as an editable dependency;
- `switchyard-langchain` from this example;
- Deep Agents and the dedicated `langchain-openrouter` integration;
- the example's development tools.

For an application that only needs the generic LangChain adapter and supplies its own provider
integration:

```bash
python -m pip install ./examples/experimental/langchain-deepagents
```

To include Deep Agents:

```bash
python -m pip install "./examples/experimental/langchain-deepagents[deepagents]"
```

To include Deep Agents, OpenRouter, and `.env` loading:

```bash
python -m pip install "./examples/experimental/langchain-deepagents[openrouter]"
```
...
The runnable example deliberately reads the repository-root `.env`, not an environment file inside
the example directory. From the repository root, create it only if you do not already have one:

```bash
cp examples/experimental/langchain-deepagents/.env.example .env
```
...
```bash
SWITCHYARD_LANGCHAIN_E2E=1 \
uv run \
  --project examples/experimental/langchain-deepagents \
  --python 3.12 \
  --extra openrouter \
  python examples/experimental/langchain-deepagents/example.py
```
...
uv run \
  ...
  --python 3.12 \
  --extra openrouter \
  pytest examples/experimental/langchain-deepagents/tests -v
```
...
uv run \
  ...
  --python 3.12 \
  --extra openrouter \
  mypy examples/experimental/langchain-deepagents/src

uv run \
  --project examples/experimental/langchain-deepagents \
  --python 3.12 \
  --extra openrouter \
  ruff check examples/experimental/langchain-deepagents
```
...
uv run \
  ...
  --python 3.12 \
  --extra openrouter \
  pytest examples/experimental/langchain-deepagents/tests/test_e2e.py \
  -v -m e2e -o addopts=
```
...
Do not put `.env` inside `examples/experimental/langchain-deepagents`; the example intentionally
...
├── pyproject.toml                 # standalone install metadata and test config
├── uv.lock                        # reproducible Python 3.12 environment
├── src/switchyard_langchain/
...
- [Switchyard repository documentation](../../../docs/index.md)
